...
\section{Einrichtung der Arbeitsumgebung und ROS~2-Paket}

\subsection{Installation von ROS~2 Humble}
\vspace{-0.4cm}
Die Arbeit erfolgt auf Basis von Ubuntu 22.04 LTS, um die vom Praktikum vorgegebene Systemumgebung sicherzustellen. Anschließend wurde ROS~2 Humble gemäß der offiziellen Anleitung installiert.\footnote{\url{https://docs.ros.org/en/humble/Installation/Ubuntu-Install-Debs.html}} 

Die Installation erfolgte auszugsweise in folgenden Schritten:

\begin{enumerate}
	\item Konfiguration der Systemlokalisierung:
	\begin{lstlisting}
sudo locale-gen en_US en_US.UTF-8
sudo update-locale LC_ALL=en_US.UTF-8 LANG=en_US.UTF-8
	\end{lstlisting}
	
	\item Einbinden der ROS~2-Repositories und des GPG-Schlüssels:
	\begin{lstlisting}
sudo apt update
...
sudo tee /etc/apt/sources.list.d/ros2.list > /dev/null
	...
	\item Installation von ROS~2 Desktop und weiteren Tools:
	\begin{lstlisting}
sudo apt update
...
sudo apt install ros-dev-tools
	...
	\item Aktivierung der ROS-Umgebung:
	\begin{lstlisting}
...
Zusätzlich wurde das Build-Tool \textit{colcon} installiert:
\begin{lstlisting}
sudo apt install python3-colcon-common-extensions
\end{lstlisting}

\subsection{Erstellung eines ROS~2-Workspaces}

Zur strukturierten Organisation eigener Pakete wurde ein Arbeitsbereich eingerichtet:
...
Die Installation von ROS~2~Humble verlief nach anfänglicher Konfiguration der Paketquellen und Systemlokalisierung problemlos. Durch die Nutzung der offiziellen Ubuntu-Repositorys konnte eine stabile und vollständig kompatible Entwicklungsumgebung eingerichtet werden. 

Mit der Einrichtung des ROS~2-Workspaces und der dauerhaften Aktivierung der ROS-Umgebungsvariablen in der \textit{.bashrc}-Datei war die Grundlage für die Entwicklung und Ausführung eigener Pakete geschaffen. Darüber hinaus wurde mit der Installation des Build-Systems \textit{colcon} eine einheitliche und reproduzierbare Build-Struktur sichergestellt.

Damit war die technische Umgebung vollständig vorbereitet, um im weiteren Verlauf die Simulationsaufgaben, die Micro-ROS-Kommunikation, die Integration des ESP32 und die praktische Steuerung des Roboters umzusetzen.
...
Bei \textit{turtlesim} gibt es beispielsweise die Node \textit{turtlesim\_node}, die das grafische Fenster und die Turtle darstellt, sowie die Node \textit{teleop\_turtle}, die Tastatureingaben des Benutzers in Geschwindigkeitskommandos übersetzt.

\subsection{Start der Turtlesim-Umgebung}
Zum Starten der Umgebung werden in zwei separaten Terminals die folgenden Befehle ausgeführt:
\begin{lstlisting}
# Terminal 1: startet die Simulation
ros2 run turtlesim turtlesim_node

# Terminal 2: startet die Steuerung per Tastatur
...
\begin{figure}[ht!]
	\centering
	\includegraphics[width=0.5\linewidth]{Abbildungen/Turtledreieck.png}
...
\begin{figure}[ht]     
	\centering
	\includegraphics[width=1\linewidth]{Abbildungen/Blockdiagramm Systemarchitektur.png} 	% nach "widh = 1" kann ein ", height=6cm" eingefügt werden um die Höhe zu verändern
